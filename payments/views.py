...
def paymentComplete(request):
    ...
    if is_ajax or request.method == 'POST':
        ...
        if not invoice_code:
            return JsonResponse({'message': 'No active invoice session.'}, status=400)
        invoice = Invoice.objects.get(invoice_code=invoice_code)
        invoice.payment_complete = True
        invoice.save()
...
def gopay_payment(request):
    ...
    if request.method == 'POST':
        ...
        if not all((settings.GOPAY_GOID, settings.GOPAY_CLIENT_ID, settings.GOPAY_CLIENT_SECRET)):
            return JsonResponse({"message": "GoPay is not configured."}, status=503)
